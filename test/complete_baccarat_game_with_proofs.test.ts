...
import { MAX_PLAYERS } from '../src/baccarat';
import { hostShowdownCoefficients } from './helpers/host-showdown-coefficients';
import { Game } from '../src/baccarat-game';
import { Player, loadBaccaratCircuits, type BaccaratCircuits } from '../src/baccarat-player';
import { verify } from '../src/rapidsnark-ffi';
import path from 'path';

const showdownVkPath = path.join(
  __dirname,
  '../zkey/baccarat_showdown_hashout_main_verification_key.json',
);

async function register(nActualPlayers: number, circuits: BaccaratCircuits, game: Game) {
...
let circuits: BaccaratCircuits;

beforeAll(async () => {
  circuits = await loadBaccaratCircuits();
});
...
test('Complete Baccarat Game with 2 players', async () => {
  await runGame(2);
}, 120_000);

test('Complete Baccarat Game with 3 players', async () => {
  await runGame(3);
}, 180_000);

test('Complete Baccarat Game with 10 players', async () => {
  await runGame(10);
}, 600_000);

test('Complete Baccarat Game with 12 players', async () => {
  await runGame(12);
}, 900_000);

test('Baccarat multi-seat player and dealer pools settle proportionally', async () => {
  const game = new Game();
  await game.init();
  ...
  game.start(
    ...
    [{ seatIndex: 0, stake: 15n }, { seatIndex: 2, stake: 5n }],
    [{ seatIndex: 1, stake: 2n }, { seatIndex: 3, stake: 1n }],
  );
  ...
  const nets = game.settleHand(1);
  expect(nets.get(0)).toBe(-15n);
  expect(nets.get(2)).toBe(-5n);
  expect(nets.get(1)).toBe(13n);
  expect(nets.get(3)).toBe(7n);
}, 600_000);