import { Baccarat, TOTAL_CARDS } from './baccarat';
import {
  type PlayerKey,
  type PublicKey,
  type Ciphertext,
...
export interface BaccaratCircuits {
  register: Circuit;
  shuffle: Circuit;
  share: Circuit;
  showdown: Circuit;
...
export async function loadBaccaratCircuits(shoeDecks: 1 | 6 | 8 = 1): Promise<BaccaratCircuits> {
  const { GameKind, normalizeGameConfig, shuffleCircuitId } = await import('./game-config');
  const shuffleName = shuffleCircuitId(normalizeGameConfig(GameKind.Baccarat, shoeDecks));
...
export class Player {
  ...
  constructor(circuits: BaccaratCircuits) {
  ...
  generateKey() {
    ...
    if (!this.#key) this.#key = this.#baccarat.generatePlayerKey();
  }
  ...
  get padding() {
    ...
    return this.#baccarat.getPadding(this.publicKey);
  }
  ...
  private prepareShuffle(deck: Ciphertext[], publicKeys: PublicKey[]) {
    const hash = this.#baccarat.shuffleHash(deck, publicKeys);
    if (!this.#shuffleParams || this.#shuffleParams.hash !== hash) {
      const permutationMatrix = this.#baccarat.generateShufflePermutation();
      const randomness = this.#baccarat.generateShuffleRandomness();
  ...
  async shuffleProve(deck: Ciphertext[], publicKeys: PublicKey[]): Promise<ShuffleOutput> {
    ...
    const { ciphertexts, permutationHash } = this.#baccarat.shuffle(
      deck,
      shuffleKeys,
  ...
  private prepareShare(deck: Ciphertext[], publicKeys: PublicKey[], nActualPlayers: number) {
    if (!this.#shareParams || this.#shareParams.nActualPlayers !== nActualPlayers) {
      const hash = this.#baccarat.shareHash(deck, this.publicKey, publicKeys);
      const randomness = new Array(publicKeys.length).fill(0n)
        .map(() => new Array(TOTAL_CARDS).fill(0n)
          .map(() => this.#baccarat.getRandom()));
  ...
  async shareProve(deck: Ciphertext[], publicKeys: PublicKey[], nActualPlayers: number): Promise<ShareOutput> {
    ...
    const params = this.prepareShare(deck, publicKeys, nActualPlayers);
    const ciphertexts = this.#baccarat.share(
      deck,
      publicKeys,
      this.#privateKey,
      params.randomness,
  ...
  private prepareShowdown(
    ...
    if (!this.#showdownParams) {
      ...
      const hash = this.#baccarat.showdownHash(hashInput);
      const plaintextCards = this.#baccarat.decryptCards(
        this.#privateKey,
        ciphertextCards,
        ciphertextPartials,
      );
      const winner = BigInt(this.#baccarat.evaluateOutcome(plaintextCards));
